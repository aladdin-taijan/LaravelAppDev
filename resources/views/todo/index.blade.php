...
<head>
    ...
    <link rel="stylesheet" href="{{asset('todo/css/styles.css')}}">
...
<body>
    ...
    @foreach($todos as $todo)
        ...
        @else
        <li id="{{$todo->id}}"><a href="#" onClick="task_done('{{$todo->id}}');"class="toggle">
        <span id="span_{{$todo->id}}">{{$todo->title}}</span></a> 
        <a href="#" onClick="delete_task('{{$todo->id}}');" class="icon-delete">Delete</a>
        <a href="#" onClick="edit_task('{{$todo->id}}','{{$todo->title}}');"class="icon-edit">Edit</a></li>
        @endif
    @endforeach
        ...
        <form method="POST" id="add_task" class="todo" style="display:none">
            @csrf
        ...
        <input id="edit_task_title" type="text" name="etitle" value="" />
        <button name="submit">Edit Task</button>
        </form>
        </section>
        </div>
        <script src="http://code.jquery.com/jquery-latest.min.js"type="text/javascript"></script>
        <script src="{{asset('todo/js/todo.js')}}" type="text/javascript"></script>
    </body>
    </html>